        ...
        For Each sLine In xStrLine
            ...
            For xI1 = 8 To Len(sLineTrim) - 1 Step 2   'For all Ks within that line ( - 1 can be ommitted )
                ...
                With Notes(UBound(Notes))
                    ...
                    If Channel = "03" Then .Value = System.Convert.ToInt32(Mid(sLineTrim, xI1, 2), 16) * 10000
            ...
            For i = 0 To UBound(relativeMeasurePos)        'assign K texts
                If CInt(relativeMeasurePos(i) / xGCD) > UBound(xStrKey) Then
                    ...
                    If BMSChannelList(CurrentBMSChannel) = "08" Then _
                        xprevNotes(UBound(xprevNotes)).Value = IIf(BPMx1296, hBPM(C36to10(NoteStrings(i))), hBPM(System.Convert.ToInt32(NoteStrings(i), 16)))
                    If BMSChannelList(CurrentBMSChannel) = "09" Then _
                        xprevNotes(UBound(xprevNotes)).Value = IIf(STOPx1296, hSTOP(C36to10(NoteStrings(i))), hSTOP(System.Convert.ToInt32(NoteStrings(i), 16)))
...
Jump1:
    ...
    Function LaneToColumn(ByVal lane As String) As Integer
        Select Case lane
            Case "0"
                Return niA1
            Case "1"
    ...
    Function LaneToColumnBestdori(ByVal lane As Long) As Integer
        Select Case lane
            Case 1
                Return niA1
            Case 2
    ...
    Private Sub OpenBestdori(ByVal chart As Chart)
        ReDim Notes(1)
        ReDim mColumn(999)
        ReDim hWAV(1295)
        ReDim hBMP(1295)
        ...
        hWAV(5) = "skill.wav"
        hWAV(6) = "slide_a.wav"
        hWAV(7) = "slide_end_a.wav"
        hWAV(8) = "slide_end_flick_a.wav"
        hWAV(9) = "slide_b.wav"
        hWAV(&HA) = "slide_end_b.wav"
        hWAV(&HB) = "slide_end_flick_b.wav"
        hWAV(&HC) = "cmd_fever_end.wav"
        hWAV(&HD) = "cmd_fever_ready.wav"
        hWAV(&HE) = "cmd_fever_start.wav"
        hWAV(&HF) = "fever_note.wav"
        hWAV(&H10) = "fever_note_flick.wav"

        NTInput = False

        Dim LongsA, LongsB As New List(Of NoteElement)
        Dim FirstBPM As Boolean = True

        For Each note In chart.Post.Notes
            If note.Type = PurpleType.System Then
                Select Case note.Cmd
                    Case CmdEnum.Bpm
                        If FirstBPM Then
                            FirstBPM = False
                            THBPM.Value = note.Bpm

                            With Notes(0)
                                .ColumnIndex = niBPM
                                .Value = note.Bpm * 10000
                                .VPosition = -1
                            End With
                        Else
                            ReDim Preserve Notes(Notes.Length)
                            With Notes(Notes.Length - 1)
                                .ColumnIndex = niBPM
                                .Value = note.Bpm * 10000
                                .VPosition = note.Beat * 48
                            End With
                        End If

                End Select
            Else
                Select Case note.Note
                    Case NoteEnum.Single
                        ReDim Preserve Notes(Notes.Length)
                        With Notes(Notes.Length - 1)
                            .ColumnIndex = LaneToColumnBestdori(note.Lane)
                            .Value = IIf(note.Flick IsNot Nothing And note.Flick = True, 4, 3) * 10000
                            .VPosition = note.Beat * 48
                        End With
                    Case NoteEnum.Slide
                        If note.Pos = Pos.A Then
                            LongsA.Add(note)
                        Else
                            LongsB.Add(note)
                        End If

                        If note.End IsNot Nothing And note.End = True Then
                            Dim nlist As List(Of NoteElement) = IIf(note.Pos = Pos.A, LongsA, LongsB)

                            For Each lnote In IIf(note.Pos = Pos.A, LongsA, LongsB)
                                ReDim Preserve Notes(Notes.Length)
                                With Notes(Notes.Length - 1)
                                    .ColumnIndex = LaneToColumnBestdori(lnote.Lane)
                                    .Value = (IIf(lnote.Pos = Pos.A, 6, 9) + IIf(lnote.End IsNot Nothing And lnote.End = True, IIf(lnote.Flick IsNot Nothing And lnote.Flick = True, 2, 1), 0)) * 10000
                                    .VPosition = lnote.Beat * 48
                                End With
                            Next lnote

                            If nlist.Count = 2 And nlist(0).Lane = nlist(1).Lane Then
                                With Notes(Notes.Length - 2)
                                    .LongNote = True
                                    .Value = 30000
                                End With
                            ...
                            IIf(note.Pos = Pos.A, LongsA, LongsB).Clear()
                        End If
                End Select
            End If
        Next note
    ...
    Private Sub OpenBBB(ByVal Path As String)
        Dim sL As String
        ReDim Notes(1)
        ReDim mColumn(999)
        ReDim hWAV(1295)
        ...
        THBPM.Value = 60 * TIME_SCALE

        Dim strLines() As String = File.ReadAllLines(Path)
        Dim cTime, cBPM, cBPB As Single, cTU As Integer

        For Each sL In strLines
            If Not sL.Contains("|") Then Continue For

            Dim parts() As String = sL.Split("|")

            Select Case parts(0)
                Case "+" 'Time Point
                    cTime = Single.Parse(parts(1)) * TIME_SCALE * 40 / 0.8333333
                    If Math.Abs(cTime - Math.Round(cTime)) < 0.001 Then cTime = Math.Round(cTime)
                    cBPM = Single.Parse(parts(2))
                    cBPB = Single.Parse(parts(3))
                    cTU = 600.0F / (cBPM * cBPB * 6) * TIME_SCALE * 4 / 0.8333333
                Case "s" 'Single
                    Dim nInfo As String() = parts(1).Split(":")

                    ReDim Preserve Notes(Notes.Length)
                    With Notes(Notes.Length - 1)
                        .ColumnIndex = LaneToColumn(nInfo(1))
                        .Value = 30000
                        .VPosition = cTime + Single.Parse(nInfo(0)) * cTU
                    End With
                Case "f" 'Flick
                    Dim nInfo As String() = parts(1).Split(":")
                    ...
                    End With
                Case "l" 'Longs
                    Dim EndFlick As Boolean = (parts(1) = "1")
                    Dim LastLane As Integer = -1

                    Dim BaseTickType As Integer = 60000
                    Dim FirstPos As Double = cTime + Single.Parse(parts(2).Split(":")(0)) * cTU
                    Dim AStartPos = -1.0, BStartPos = -1.0
                    For Each note In Notes
                        Select Case note.Value
                            Case 60000
                                If AStartPos = -1.0 Then AStartPos = note.VPosition
                            Case 70000, 80000
                                If FirstPos >= AStartPos And FirstPos <= note.VPosition Then
                                    BaseTickType = 90000
                                    Exit For
                                End If
                    ...
                    For i = 2 To parts.Length - 1
                        Dim nInfo() As String = parts(i).Split(":")

                        ReDim Preserve Notes(Notes.Length)
                        With Notes(Notes.Length - 1)
                            .ColumnIndex = LaneToColumn(nInfo(1))
                            .Value = BaseTickType + IIf(i = parts.Length - 1, IIf(EndFlick, 20000, 10000), 0)
                            .VPosition = cTime + Single.Parse(nInfo(0)) * cTU
                        End With
                        ...
                        '    If LastLane = -1 Then
                        '        LastLane = LaneToColumn(nInfo(1))
                        '    ElseIf LastLane = LaneToColumn(nInfo(1)) Then
                        '        With Notes(Notes.Length - 1)
                        '            .Value = IIf(EndFlick, 40000, 30000)
                        '            .LongNote = True
                        '        End With
