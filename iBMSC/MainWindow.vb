...
Imports VGAudio.Containers
Imports VGAudio.Containers.Hca
Imports VGAudio.Containers.Wave
Imports VGAudio.Codecs.CriHca
...
case2:              Dim xI0 As Integer
    ...
    Private Sub ImportFromBangBangBoomToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportFromBangBangBoomToolStripMenuItem.Click
        ReDim SelectedNotes(-1)
        KMouseOver = -1
        If ClosingPopSave() Then Return

        Dim xDOpen As New OpenFileDialog
        xDOpen.Filter = Strings.FileType.BangBangBoom & "|*.txt"
        xDOpen.DefaultExt = "txt"
        xDOpen.InitialDirectory = IIf(ExcludeFileName(FileName) = "", InitPath, ExcludeFileName(FileName))

        If xDOpen.ShowDialog = Windows.Forms.DialogResult.Cancel Then Return
        InitPath = ExcludeFileName(xDOpen.FileName)
        SetFileName("Imported_" & GetFileName(xDOpen.FileName))
        OpenBBB(xDOpen.FileName)
        NewRecent(xDOpen.FileName)
        SetIsSaved(False)
    End Sub

    Const GarupaKey As Integer = 8190

    Private Sub EncodeHCAToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles EncodeHCAToolStripMenuItem.Click
        Dim xDOpen As New OpenFileDialog
        xDOpen.Filter = "Wave Sound|*.wav"
        xDOpen.DefaultExt = "wav"
        xDOpen.InitialDirectory = IIf(ExcludeFileName(FileName) = "", InitPath, ExcludeFileName(FileName))

        If xDOpen.ShowDialog = Windows.Forms.DialogResult.Cancel Then Return

        Dim reader As New WaveReader()
        Dim writer As New HcaWriter() With {
            .Configuration = New HcaConfiguration() With {
                .Bitrate = 48000, .EncryptionKey = New CriHcaKey(8910UL), .Quality = CriHcaQuality.Highest, .TrimFile = False
            }
        }

        Dim audioData As VGAudio.Formats.AudioData

        Using fread = File.OpenRead(xDOpen.FileName)
            audioData = reader.Read(fread)
        End Using

        Using stream = File.Open(xDOpen.FileName.Replace(".wav", ".hca"), FileMode.OpenOrCreate)
            writer.WriteToStream(audioData, stream)
        End Using

        MessageBox.Show("Convert done! Output path is " + xDOpen.FileName.Replace(".wav", ".hca"), "Garupa Tools", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub

    Private Sub DecodeHCAToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DecodeHCAToolStripMenuItem.Click
        Dim xDOpen As New OpenFileDialog
        xDOpen.Filter = "Criware HCA Sound File|*.hca"
        xDOpen.DefaultExt = "hca"
        xDOpen.InitialDirectory = IIf(ExcludeFileName(FileName) = "", InitPath, ExcludeFileName(FileName))

        If xDOpen.ShowDialog = Windows.Forms.DialogResult.Cancel Then Return

        Dim reader As New HcaReader() With {
            .EncryptionKey = New CriHcaKey(8910UL)
        }
        Dim writer As New WaveWriter()
        ...
        Using stream = File.Open(xDOpen.FileName.Replace(".hca", ".wav"), FileMode.OpenOrCreate)
            writer.WriteToStream(audioData, stream)
        End Using

        MessageBox.Show("Convert done! Output path is " + xDOpen.FileName.Replace(".hca", ".wav"), "Garupa Tools", MessageBoxButtons.OK, MessageBoxIcon.Information)
    End Sub

    Private Async Sub ImportFromBestdoriToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ImportFromBestdoriToolStripMenuItem.Click
        Dim id As String = InputBox("Please input bestdori chart id", "Bestdori Importer")

        Using webc As New WebClient()
            webc.Encoding = System.Text.Encoding.UTF8
            Dim chart As String = Await webc.DownloadStringTaskAsync($"https://bestdori.com/api/post/details?id={id}")
            OpenBestdori(Bestdori.Chart.FromJson(chart))
            SetIsSaved(False)
        End Using
    End Sub

    Private Sub AddRandomSkillNotesToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AddRandomSkillNotesToolStripMenuItem.Click
        Dim ub = (Notes.Count() * 0.8)
        Dim sb = Int(Notes.Count() * 0.1)

        Dim r As New Random()
        For i = 1 To 6
            If Notes(sb).Value = 30000 Then
                Notes(sb).Value = 50000
                sb += Int(ub / 6)
            Else
                i -= 1
                sb += 1 
